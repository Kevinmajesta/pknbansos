    ...
    <div class="row-fluid">
      <form id="frm" method="post" action="<?php echo base_url(); ?>proposal/<?php echo $link_proses;?>">
        ...
        <fieldset class="span9">
          ...
          <fieldset>
            <legend></legend>
            <div class="controls-row" data-bind="validationElement: nik,visible: nik_vis">
            ...
            <div class="controls-row" data-bind="visible: nama_pmh_vis">
              <div class="control-group">
                ...
                <input type="text" class="span10" id="nama_pmh" data-bind="value: nama_pmh, enable: nama_pmh_vis" maxlength="100" />
              </div>
            </div>
            <div class="controls-row" data-bind="visible: alamat_pmh_vis">
              <div class="control-group">
                ...
                <textarea class="span10" rows="3" id="alamat_pmh" data-bind="value: alamat_pmh, enable: alamat_pmh_vis" maxlength="200"></textarea>
              </div>
            </div>
...
$(document).ready(function() {
  ...
  //promise untuk mengecek nomer proposal
  App.getduplikat_no = function () {
	return new Promise(function (resolve, reject) {
				var ajax = $.ajax({
			type: "post",
			dataType: "json",
			url: root+modul+'/duplikat_no/',
			data: {no: App.no(),id:App.id()},
			success: function(res, xhr){
				//console.log(res);
 ...
 App.getduplikat_nik = function () {
	return new Promise(function (resolve, reject) {
		  ...
		  ajax.done(function(data) {
			 ...
			 answer = confirm('NIK tersebut sudah pernah mengajukan proposal pada tanggal '+tanggal+'. Lanjutkan entri data dengan NIK tersebut?');		
							if(answer == true){
							resolve(answer);
							return true;
							}
  ...
  App.save = function(createNew){
	...
	App.getduplikat_no()
	  .then(function (data) {
	  	App.getduplikat_nik()
			.then(function (data) {
		// lakukan sesuatu setelah promise = true
		 console.log('promise jalan');
		 var $frm = $('#frm'),
