...
jQuery(document).ready(function() {
	...
	jQuery("#gridSKPD").jqGrid({
		url:'<?php echo base_url()?>skpd/get_skpd_dpalanjut',
		datatype: "json",
        mtype:'POST',
		...
		ondblClickRow:function(){
			...
			jQuery.getJSON('<?php echo base_url();?>dpa_skpdlanjut/get_pejabat/'+id_skpd, function(json) {
			...
			jQuery.getJSON('<?php echo base_url();?>dpa_skpdlanjut/get_pejabat_daerah/', function(json) {
			...
			function pilih_kegiatan()
			{
				...
				if(idk)
				{
					...
					jQuery.getJSON('<?php echo base_url();?>dpa_skpdlanjut/cek_murni/'+skpd+'/'+kegiatan, function(json){
						if(json.rows.status == true)
						{
							jQuery('#tableIndikatorKinerja').jqGrid('setGridParam', {datatype:'json'});
							jQuery('#tableIndikatorKinerja').jqGrid('setGridParam', {url:'<?php echo base_url();?>dpa_skpdlanjut/get_daftar_indikator/' + json.rows.form + '/' + true})
							...
							jQuery('#tableLanjutanPelaksanaan').jqGrid('setGridParam', {datatype:'json'});
							jQuery('#tableLanjutanPelaksanaan').jqGrid('setGridParam', {url:'<?php echo base_url();?>dpa_skpdlanjut/get_daftar_Lanjutan/' + json.rows.form+ '/' + true})
							jQuery('#tableLanjutanPelaksanaan').jqGrid('setGridParam', {gridComplete:function(){
									var idr = jQuery('#tableLanjutanPelaksanaan').jqGrid('getDataIDs');
									for(var i=0; i<idr.length; i++)
									{
										jQuery('#tableLanjutanPelaksanaan').jqGrid('setCell', idr[i], 'ID_FORM_ANGGARAN', null, null, null, true);
									}
								}
								}).trigger('reloadGrid');
								
							jQuery('#tableRincianAnggaran').jqGrid('setGridParam', {datatype:'json'});
							jQuery('#tableRincianAnggaran').jqGrid('setGridParam', {url:'<?php echo base_url();?>dpa_skpdlanjut/get_daftar_rincian/' + json.rows.form+ '/' + true})
							...
							jQuery('#tableAnggaranKas').jqGrid('setGridParam', {url:'<?php echo base_url();?>dpa_skpdlanjut/get_anggaran_kas/' + json.rows.form+ '/' + true})
			...
			}).load('<?php echo base_url();?>kegiatan/pilih_dpalanjut');
