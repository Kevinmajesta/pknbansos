...
class Nphd extends Admin_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('nphd_model', 'data_model');
		$this->load->model('auth/login_model', 'auth');
	}

	public function index()
	{
		$data['breadcrumbs'] = 'Daftar NPHD';
		$data['title'] = PRODUCT_NAME.' - '.$data['breadcrumbs'];
		$data['link_daftar'] = '/get_daftar';
		$data['modul'] = 'nphd';
		$data['main_content'] = 'nphd_view';
	...
	public function proses()
	{
		...
		$this->form_validation->set_rules('no', 'Nomor NPHD', 'required|trim|max_length[100]|callback__cek_nomor');
		$this->form_validation->set_rules('tgl', 'Tanggal', 'required|trim');
		$this->form_validation->set_rules('nik', 'NIK', 'required|trim');
		$this->form_validation->set_rules('nama', 'Nama', 'required|trim');
		$this->form_validation->set_rules('pekerjaan', 'Instansi/Organisasi', 'required|trim');
		$this->form_validation->set_rules('mewakili', 'Jabatan', 'required|trim');
	...
	public function hapus()
	{
		...
		$response->isSuccess = TRUE;
		$response->message = 'Data nphd telah dihapus.';
	...
	{
		$result = $this->data_model->get_rincian($id_skpd,$id_rekening,$id_rincian_anggaran);		
		$nphd_pakai = $this->data_model->total_nphd($id,$id_rekening);
		$response = (object) NULL;
		if ($result){
			for($i=0; $i<count($result); $i++)
			{
				...
				$response->rows[$i]['cell']=array(
					...
					($result[$i]['LEVEL']==2)?(($result[$i]['NOMINAL_NPHD']>0)?$result[$i]['NOMINAL_NPHD']:$result[$i]['PAGU']):$result[$i]['PAGU'],
					//($result[$i]['PAGU']-$nphd_pakai),
					0,
					($result[$i]['LEVEL']==2)?$result[$i]['PAGU']:($result[$i]['PAGU']-$nphd_pakai),
					$result[$i]['LEVEL'],
				);
			}
		}
