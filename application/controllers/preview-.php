...
class Preview extends CI_Controller{
  ...
  public function __construct()
  {
    ...
    $this->attachment = 0; // 0 : show in the browser;  1 : download file
  }
  ...
  public function view($key = '', $reportname = '')
  {
	...
	public function view2($reportname = '')
	{
		$this->load->helper('file');

		header('Content-Disposition: ' . 'filename="'.$reportname.'"');
		if ($filetype == 'pdf')
			header('Content-Type: application/pdf');
		else
			header('Content-Type: application/xls');
		...
		header('Content-Length: '.filesize($this->path2output.$filename));
		header('Cache-Control: no-store');
		readfile($this->path2output.$reportname);
  ...
  function kontra_pos(){
    ...
    switch ($tipe){
      case 'daftar' :
		  ...
		  $param['m'] = $m;
          // ambil query daftar
          $this->load->model('kontra_pos/kontra_model','data_model');
          $query_str = $this->data_model->get_data($param, FALSE, TRUE);
          $query_str = preg_replace("/[\n\r]/"," ", $query_str);
          $param['opt'] = array(
            "SQLFilter=".$query_str
  ...
  function setoran_sisa(){
    ...
    switch ($tipe){
      case 'daftar' :
		  ...
		  $param['m'] = $m;
          // ambil query daftar
          $this->load->model('setoran_sisa/setoran_model','data_model');
          $query_str = $this->data_model->get_data($param, FALSE, TRUE);
          $query_str = preg_replace("/[\n\r]/"," ", $query_str);
          $param['opt'] = array(
            "SQLFilter=".$query_str
  ...
  function rekening_bendahara(){
	...
	function bast(){
		...
		switch ($tipe){
		  case 'daftar' :
			  ...
			  break;
		  case 'form' :
			  $param['fr3'] = 'Shared\Rpt_Form_BAST.fr3';
	...
	function sumber_dana(){
		...
		switch ($tipe){
			case 'daftar' :
				$param['fr3'] 				= 'Shared\DaftarSumberDana.fr3';
				$param['nama']				= 'Daftar Sumber Dana';
				$param['format'] 			= $format;
				$param['search'] 			= $search;
				$param['search_field'] 		= $search_field;
				$param['search_operator'] 	= $search_operator;
				...
				$param['limit'] 			= $limit;
				// ambil query daftar
				$this->load->model('sumber_dana/sumberdana_model','data_model');
				$query_str = $this->data_model->get_data($param, FALSE, TRUE);
				$query_str = preg_replace("/[\n\r]/"," ", $query_str);
				$param['opt'] = array(
					"SQLFilter=".$query_str
	...
	function tim_anggaran(){
		...
		switch ($tipe){
			case 'daftar' :
				$param['fr3'] 				= 'Shared\Tim_Anggaran.fr3';
				$param['nama']				= 'Daftar Tim Anggaran';
				$param['format'] 			= $format;
				$param['search'] 			= $search;
				$param['search_field'] 		= $search_field;
				$param['search_operator'] 	= $search_operator;
				...
				$param['limit'] 			= $limit;
				// ambil query daftar
				$this->load->model('tim_anggaran/tim_anggaran_model','data_model');
				$query_str = $this->data_model->get_data($param, FALSE, TRUE);
				$query_str = preg_replace("/[\n\r]/"," ", $query_str);
				$param['opt'] = array(
					"SQLFilter=".$query_str
	...
	function kontrak()
	{
		...
		switch ($tipe){
		  case 'daftar' :
			  $param['fr3'] = 'Shared\DaftarKontrak.fr3';
			  $param['nama'] = 'Daftar Kontrak';
			  $param['format'] = $format;
			  $param['q'] = $q;
			  $param['m'] = $m;
			  // ambil query daftar
			  $this->load->model('kontrak/kontrak_model','data_model');
			  $query_str = $this->data_model->get_data($param, FALSE, TRUE);
			  $query_str = preg_replace("/[\n\r]/"," ", $query_str);
			  $param['opt'] = array(
				"SQLFilter=".$query_str
	...
	switch ($tipe){
			case 'daftar' :
				$param['fr3'] 				= 'Shared\DaftarTimPenguji.fr3';
				$param['nama']				= 'Daftar Tim Penguji';
				$param['format'] 			= $format;
				$param['search'] 			= $search;
				$param['search_field'] 		= $search_field;
				$param['search_operator'] 	= $search_operator;
				...
				$param['limit'] 			= $limit;
				// ambil query daftar
				$this->load->model('tim_penguji/timpenguji_model','data_model');
				$query_str = $this->data_model->get_data($param, FALSE, TRUE);
				$query_str = preg_replace("/[\n\r]/"," ", $query_str);
				$param['opt'] = array(
					"SQLFilter=".$query_str
