...
$(document).ready(function() {
	...
	$("#grid").jqGrid({
		...
		datatype:'json',
		mtype:'POST',
		colNames:['Nomor BAST', 'Tanggal', 'Nama', 'Peruntukan', 'Keperluan', 'Nominal'],
		colModel:[
			{name:'no', width:100, align:'center'},
			{name:'tgl', width:100, align:'center', formatter:'date', sortable:false},
			{name:'nama', width:250},
			{name:'peruntukan', width:150},
			{name:'keperluan', width:100},
			{name:'nominal', width:150, align:'center', formatter:'currency', align:'right', sortable:false},        
		],
		pager:'#pager',
		rowNum:10,
		rowList:[10,20,30],
  ...
  {		
		var SumNom = $("#grid").jqGrid('getCol', 'nominal', false, 'sum');
    $("#grid").jqGrid('footerData','set',{akhir:'',nominal:SumNom});
  }
