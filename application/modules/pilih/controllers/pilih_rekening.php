	...
	*/
		...
		if ($result){
			for($i=0; $i<count($result); $i++){
				...
				$response->rows[$i]['cell'] = array(
					...
					utf8_encode($result[$i]['KODE_REKENING']),
					utf8_encode($result[$i]['NAMA_REKENING']),
				);
			}
		}
		$response->sql = $this->db->queries;
		echo json_encode($response); //die(print_r( json_last_error()));
	}
	...
	function getrekeningUraian()
	{
		...
		if ($result){
			$no = 0;
			for($i=0; $i<count($result); $i++){
				if($result[$i]['JUMLAH_BAST'] < $result[$i]['PAGU']){
					$response->rows[$no]['id'] = $result[$i]['ID_RINCIAN_ANGGARAN'];
					$response->rows[$no]['cell'] = array(
						$result[$no]['ID_RINCIAN_ANGGARAN'],
						$result[$no]['ID_REKENING'],
						$result[$no]['ID_PARENT_REKENING'],
						$result[$no]['LEVEL_REKENING'],
						$result[$no]['KODE_REKENING'],
						$result[$no]['URAIAN'],
						$result[$no]['PAGU']
					);
					$no+=1;
