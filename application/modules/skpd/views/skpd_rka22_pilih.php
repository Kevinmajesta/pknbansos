...
jQuery(document).ready(function() {
	...
	jQuery("#gridSKPD").jqGrid({
		url:'<?php echo base_url()?>skpd/get_skpd_rka22',
		datatype: "json",
        mtype:'POST',
        colNames:['Kode SKPD', 'Nama SKPD'],
        colModel:[	{name:'KODE_SKPD_LKP', index:'KODE_SKPD_LKP', width:30},
					{name:'nama', index:'nama', width:140,}],
        //rowNum:10,
		rowNum:10,
		rownumbers:true,
        pager: '#pagerSKPD',
		sortname:'KODE_SKPD_LKP',
		sortorder: "asc",
        viewrecords: true,
        gridview:true,
        width:'525',
		...
		ondblClickRow:function(){
			...
			var KODE_SKPD_LKP = rs.KODE_SKPD_LKP;
			var nama = rs.nama;
			jQuery('#ID_SKPD').val(id);
			jQuery('#KODE_SKPD').val(KODE_SKPD_LKP);
			...
			jQuery.getJSON('<?php echo base_url();?>rka_skpd1/get_pejabat/'+id_skpd, function(json) {
