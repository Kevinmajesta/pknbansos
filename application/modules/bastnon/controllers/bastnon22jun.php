...
class Bastnon extends Admin_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('bastnon_model', 'data_model');
		$this->load->model('auth/login_model', 'auth');
	}

	public function index()
	{
		$data['breadcrumbs'] = 'Daftar BAST Bantuan Non Proposal';
		$data['title'] = PRODUCT_NAME.' - '.$data['breadcrumbs'];
		$data['link_daftar'] = '/get_daftar';
		$data['modul'] = 'bastnon';
		$data['main_content'] = 'bastnon_view';
	...
	{
		$data['title'] = PRODUCT_NAME.' - '.'BAST - Bantuan Non Proposal';
		$data['modul'] = 'bastnon';
		...
		$data['main_content']='bastnon_form';
		$this->load->view('layout/template',$data);
	}
		...
		if ($result){
			for($i=0; $i<count($result); $i++)
			{
				...
				$response->rows[$i]['cell']=array(
					...
					0,
					($result[$i]['LEVEL']==2)?$result[$i]['PAGU']:($result[$i]['PAGU']-$bast_pakai),
					$result[$i]['LEVEL'],
				);
			}
		}
