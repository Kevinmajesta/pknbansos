...
<form id="frm" method="post" action="<?php echo base_url(); ?>nphd/<?php echo $link_proses;?>" enctype="multipart/form-data">
	<fieldset>
		<div class="controls-row">
			<div class="control-group pull-left" data-bind="validationElement: no" >
				<label class="control-label" for="no">Nomor NPHD</label>
		...
		<div class="controls-row">	
			<div class="control-group pull-left" >
				<label class="control-label" for="uraian">Kegiatan</label>
		...
		<div class="controls-row">
			<div class="control-group pull-left" data-bind="validationElement: id_sp2d" >
			  <label class="control-label" for="no_sp2d">SP2D</label>
			  <div class="controls input-append">
				<input type="text" class="span5" id="no_sp2d" readonly="1" data-bind="value: no_sp2d, executeOnEnter: pilih_sp2d" required />
				<span class="add-on" data-bind="click: pilih_sp2d, visible: !isEdit() && canSave()" ><i class="icon-folder-open"></i></span>
			  </div>
			</div>
		</div>
	</fieldset>
	...
	<fieldset>
		<legend>Data Penerima Hibah</legend>
		<div class="controls-row">
			<div class="control-group pull-left" data-bind="validationElement: nama">
				<label class="control-label" for="nama">Nama Pemerima</label>
		...
		<div class="controls-row">
			<div class="control-group pull-left" data-bind="validationElement: pekerjaan" >
				<label class="control-label" for="pekerjaan">Instansi/Organisasi</label>
			...
			<div class="control-group pull-left span2" data-bind="validationElement: mewakili" >
				<label class="control-label" for="mewakili">Jabatan</label>
				<input type="text" id="mewakili" class="span3" data-bind="value: mewakili, select2: { minimumResultsForSearch: -1, containerCss: {'margin-left':'0px'}, containerCssClass: 'span3',  placeholder: 'Jabatan',  data:{ results: data_mewakili}, }" />
			</div>			
...
<script>
var data_mewakili = [{id:1, text:'Ketua'}, {id:2, text:'Wakil Ketua'}, {id:3, text:'Sekretaris'}];
...
$(document).ready(function() {
	...
	var ModelBAST = function (){
		...
		self.pekerjaan = ko.observable(<?php echo isset($data['PEKERJAAN']) ? json_encode($data['PEKERJAAN']) : "''" ?>)
		  .extend({
			required: {params: true, message: 'Instansi/Organisasi belum diisi'},
			maxLength: {params: 100, message: 'Instansi/Organisasi tidak boleh melebihi 100 karakter'},
		  });
		self.mewakili = ko.observable('<?php echo isset($data['MEWAKILI']) ? $data['MEWAKILI'] : '' ?>')
		  .extend({
			required: {params: true, message: 'Jabatan belum dipilih'}
		  });
		...
		self.id_sp2d = ko.observable(<?php echo isset($data['ID_SP2D']) ? $data['ID_SP2D'] : '' ?>)
		  .extend({
			required: {params: true, message: 'SP2D belum dipilih'}
		  });
		self.no_sp2d = ko.observable(<?php echo isset($data['NOMOR_SP2D']) ? json_encode($data['NOMOR_SP2D']) : '' ?>);
		...
		self.title = ko.computed(function(){
		  return (self.mode() === 'edit' ? 'Edit ' : 'Entri ') + ' NPHD';
		});
	...
	App.pilih_uraian = function(){
		...
		var option = {multi:0, lvl:0, tree:2, id_skpd:App.id_skpd(), mode:'NPHD'};
	...
	App.init_grid = function(){
		...
		$.ajax({
			...
			success: function(res) {				
				App.sisa_sp2d_asli(parseFloat(res.nominal_sp2d-res.nphd_pakai));
				App.sisa_sp2d(parseFloat(res.nominal_sp2d-res.nphd_pakai-App.nom()));
			},
		});	
	...
	App.pilih_sp2d = function(){
		...
		var option = {multi:0, mode:'nphd', id_skpd:App.id_skpd()};
