...
class Nphd extends Admin_Controller {
	...
	function rincian($id = 0,$id_skpd = 0,$id_rekening = 0,$id_rincian_anggaran = 0)
	{
		...
		$nphd_pakai = $this->data_model->total_nphd($id,$id_rekening,$id_rincian_anggaran);
		$all_nphd_pakai = $this->data_model->total_nphd($id,$id_rekening,0);
		$response = (object) NULL;
		if ($result){
			for($i=0; $i<count($result); $i++)
			{
				...
				$response->rows[$i]['cell']=array(
					...
					($result[$i]['LEVEL']=='2')?(($id>0)?$result[$i]['NOMINAL_BAST']:$result[$i]['PAGU']-$nphd_pakai):$result[$i]['PAGU'],
					0,
					($result[$i]['LEVEL']=='2')?$result[$i]['PAGU']-$nphd_pakai:$result[$i]['PAGU']-$all_nphd_pakai,
					$result[$i]['LEVEL'],
				);
			}
		}
