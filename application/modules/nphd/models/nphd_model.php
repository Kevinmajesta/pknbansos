<?php
class Nphd_model extends CI_Model {
	...
	function get_data_by_id($id)
	{
		...
		$this->db->join('rincian_anggaran e', 'e.id_rincian_anggaran = a.id_rincian_anggaran');
		$this->db->join('v_skpd d', 'd.id_skpd = a.id_skpd');
		$this->db->join('v_sp2d_kegiatan f', 'f.id_aktivitas = a.id_sp2d');
	...
	{		
		...
		");
		$this->db->from('bast b');
		$jenis = array('BANKEU','BANSOS','NPHD');
		...
		$hasil = array('nama_rekening'=>$anggaran['NAMA_REKENING'], 'total_pagu'=>$anggaran['TOTAL_PAGU_REKENING'], 'nphd_pakai'=>$bast['BAST_PAKAI'], 'nominal_sp2d'=>$sp2d['NOMINAL_SP2D']);
		return $hasil;
	}
	...
	function get_rincian($id,$id_skpd,$id_rekening,$id_rincian_anggaran)
	{
		if($id > 0){
			$result = $this->db->query("
				...
				join tahun_anggaran ta on fa.tahun=ta.tahun and ta.status_kini=fa.status	
				...
				join form_anggaran fa on fa.id_form_anggaran = ra.id_form_anggaran
				join tahun_anggaran ta on fa.tahun=ta.tahun and ta.status_kini=fa.status
				left join bast ba on ba.id_rincian_anggaran=ra.id_rincian_anggaran and ba.jenis='NPHD'
	...
	function total_nphd($id_bast,$id_rekening,$id_rincian_anggaran){
		//total bast
		$this->db->select("
			coalesce(sum(b.nominal), 0) bast_pakai
		");
		$this->db->from('bast b');
		$this->db->where('b.jenis','NPHD');
		$this->db->where('b.id_rekening', $id_rekening);
		if($id_rincian_anggaran)
			$this->db->where('b.id_rincian_anggaran', $id_rincian_anggaran);
		if($id_bast)
